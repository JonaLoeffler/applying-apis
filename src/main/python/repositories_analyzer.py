...
def analyze(repositories_selected_file: str):
    ...
    """
    print("*** Analyzing " + repositories_selected_file + ".")
...
def count_pairs_in_abstraction_and_repo(repository: str, abstraction: str, package1: str, package2: str):
    """
    Counts the occurrences of the packages `package1` and `package2` in the `repository`
    on the abstraction level `abstraction`. Returns a list that contains `repository`,
    `count_package1`, `count_package2`, and `count_both`.
    """
    sets_file = u.analyzed_data_dir + abstraction + "/" + \
    ...
    count_both = df.filter(F.array_contains(u.apis, package1) &
                           F.array_contains(u.apis, package2)).count()
    count_package1 = df.filter(
        F.array_contains(u.apis, package1)).count()
    count_package2 = df.filter(
        F.array_contains(u.apis, package2)).count()

    return [repository, count_package1, count_package2, count_both]


def count_pairs_in_abstraction(abstraction: str, repositories_selected_file: str, package1: str, package2: str):
    """
    Counts the occurrences of the packages `package1` and `package2` in all repositories
    ...
    for repository in df.select("repositoryName").rdd.flatMap(lambda x: x).collect():
        count = count_pairs_in_abstraction_and_repo(
            repository, abstraction, package1, package2)
        if count != []:
            counts.append(count)

    schema = StructType([StructField("repositoryName", StringType(), False),
                        StructField(package1, IntegerType(), False),
                        StructField(package2, IntegerType(), False),
                        StructField("both", IntegerType(), False)])
    data = spark.createDataFrame(counts, schema)

    u.delete_dir(u.spark_dir)
    u.write_csv(data.coalesce(1), u.spark_dir)
    u.copy_csv(u.spark_dir, u.analyzed_data_dir + "counts" + "/" + package1.replace(":", "_")
               + "_" + package2.replace(":", "_") + "_" + abstraction + ".csv")

    count_package1 = data.select(
        F.sum(F.col("`" + package1 + "`"))).collect()[0][0]
    count_package2 = data.select(
        F.sum(F.col("`" + package2 + "`"))).collect()[0][0]
    count_both = data.select(F.sum(F.col("both"))).collect()[0][0]
    return [abstraction, count_package1, count_package2, count_both]


def count_pairs(repositories_selected_file: str, package1: str, package2: str):
    """
    Counts the occurrences of the packages `package1` and `package2` in all repositories
    ...
    all_counts.append(count_pairs_in_abstraction(
        "method", repositories_selected_file, package1, package2))
    all_counts.append(count_pairs_in_abstraction(
        "class", repositories_selected_file, package1, package2))
    all_counts.append(count_pairs_in_abstraction(
        "package", repositories_selected_file, package1, package2))
    ...
    u.delete_dir(u.spark_dir)
    u.write_csv(data.coalesce(1), u.spark_dir)
    u.copy_csv(u.spark_dir, u.analyzed_data_dir + "counts" + "/" + "counted_" + package1.replace(":", "_")
               + "_" + package2.replace(":", "_") + ".csv")


def sample_abstractions(repositories: int, samples_in_repository: int, usage_limit: int, abstraction: str, package1: str, package2: str):
    """
    Samples (number:)`repositories` repositories that have (number:)`usage_limit` abstractions of type `abstraction`
    that contain the packages `package1` and `package2`. From each repository (number:)`samples_in_repository`
    abstractions are sampled.
    """
    sample = []
    df = u.read_csv(spark, u.analyzed_data_dir + "counts" + "/" + package1.replace(":", "_")
                    + "_" + package2.replace(":", "_") + "_" + abstraction + ".csv")
    count = df.count()
    ...
    print("Filtered out", count_filtered, "out of", count,
          "repositories with at least", usage_limit, "abstractions that use both packages.")
    ...
    for repository in sc.parallelize(df.rdd.takeSample(False, repositories)).map(tuple).collect():
        ...
        data = data.withColumn(u.apis, F.split(
            F.regexp_replace(u.apis, "[\[\]]", ""), ",")) \
            .filter(F.array_contains(u.apis, package1) &
                    F.array_contains(u.apis, package2)) \
    ...
    u.copy_csv(u.spark_dir, u.analyzed_data_dir + "sampled_abstractions" + "/"
               + "sampled_abstractions_" + package1.replace(":", "_") + "_"
               + package2.replace(":", "_") + ".csv")
...
if __name__ == "__main__":
    ...
    package1 = "org.apache.lucene:lucene-analyzers-common"
    package2 = "org.apache.lucene:lucene-core"
    repositories_selected_file = u.repositories_selected_dir + \
        package1.replace(":", "_") + "_" + \
        package2.replace(":", "_") + ".csv"

    analyze(repositories_selected_file)
    count_pairs(repositories_selected_file, package1, package2)
    sample_abstractions(5, 2, 5, "method", package1, package2)
